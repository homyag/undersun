...
- **Translations**: django-modeltranslation, Rosetta UI, автоматический перевод через Yandex Translate (`TranslationService`).
- **Media**: django-imagekit for responsive thumbnails, TinyMCE for blog content.
- **Build tooling**: django-tailwind app `theme` (Tailwind 4). Use `python manage.py tailwind start` for live reload or `python manage.py tailwind build` before `collectstatic`.
...
python manage.py tailwind install  # installs Node deps inside theme/static_src
python manage.py tailwind build    # generate css/dist/styles.css
...
- **TranslationService** (`apps/core/services.py`): обертка над Yandex Translate API с чанкингом и сохранением HTML. Глобальный экземпляр доступен как `translation_service`.
...
- `TranslationService` требует конфигурации Yandex Translate (`YANDEX_TRANSLATE_API_KEY`, `YANDEX_TRANSLATE_FOLDER_ID`, опционально `YANDEX_TRANSLATE_ENDPOINT`). Утилиты в `apps/properties/services.py` показывают типовой сценарий использования.
...
- Nginx ingestion helper: `python scripts/nginx_bad_requests_ingest.py --access-log /var/log/nginx/access.log`
  читает access.log, ищет ботов по запретным паттернам (`/remote/login`, `/wp-login.php`, `.env`, `/cgi-bin`, `/aws`,
  `/phpmyadmin`, статусам `400/401/405/444`, нестандартным методам и т.д.) и дописывает находки в `logs/bad_requests.log`
  в том же формате, что и Django-middleware. Offset хранения — `logs/.nginx_bad_requests.offset`, удобно вешать на cron.
...
- `ARCHITECTURE.md` - exhaustive project overview (architecture, features, deployment checklist).
...
- When in doubt, review `ARCHITECTURE.md` for the long-form description of any subsystem.
